...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if (!in_array($new_status, $valid_statuses)) {
        ...
        try {
            ...
            // Update the request variable to show the new status
                ...
                <form method="POST" class="status-form">
                    ...
                    <div class="status-options">
                        <div class="status-option">
                            <input type="radio" id="status-pending" name="status" value="pending" <?php echo $request['status'] === 'pending' ? 'checked' : ''; ?>>
                            <label for="status-pending">รอดำเนินการ</label>
                        </div>
                        <div class="status-option">
                            <input type="radio" id="status-in-progress" name="status" value="in-progress" <?php echo $request['status'] === 'in-progress' ? 'checked' : ''; ?>>
                            <label for="status-in-progress">กำลังดำเนินการ</label>
                        </div>
                        <div class="status-option">
                            <input type="radio" id="status-completed" name="status" value="completed" <?php echo $request['status'] === 'completed' ? 'checked' : ''; ?>>
                            <label for="status-completed">เสร็จสิ้น</label>
                        </div>
                    </div>
                    <button type="submit" name="update_status"><i class="fas fa-save"></i> อัปเดตสถานะ</button>
                </form>
                ...
                <div class="comments-section">
                    ...
                    <div class="comment-form">
                        ...
                        <div class="admin-info">
                            ...
                            <span>คุณกำลังแสดงความคิดเห็นในฐานะผู้ดูแลระบบ: <strong><?php echo htmlspecialchars($_SESSION['admin_name'], ENT_QUOTES, 'UTF-8'); ?></strong></span>
                        </div>

                        <form id="commentForm">
                            <input type="hidden" name="request_id" value="<?php echo $request_id; ?>">
                            <input type="hidden" name="user_name" value="<?php echo htmlspecialchars($_SESSION['admin_name'], ENT_QUOTES, 'UTF-8'); ?>">
        ...
        $(document).ready(function() {
            ...
            $('#commentForm').on('submit', function(e) {
                ...
                if (!validateCommentForm()) {
                    $("#commentMessage").addClass("error").html("กรุณากรอกข้อความแสดงความคิดเห็น").show();
                    return;
                }
                    ...
                    success: function(response) {
                        try {
                            ...
                            if (data.status === 'success') {
                                ...
                                $('#comment').val(''); // เคลียร์เฉพาะข้อความ
